const API_URL = '/api';
const SUPABASE_URL = 'https://rfceapkvgnmzqpbgxuus.supabase.co/rest/v1';
const SUPABASE_KEY = 'sb_secret_RG5vyDUd3G6JbHb_OJlh_A_HkPrAWvp';
...
async function supabaseApiRequest(action, data = {}) {
  ...
  const supabaseActions = {
    login: async (d) => {
      console.log('supabaseApiRequest login called with uid:', d.uid);
      console.log('SUPABASE_URL:', SUPABASE_URL);
      try {
        const result = await fetch(SUPABASE_URL + '/users?uid=eq.' + d.uid, { 
          method: 'GET',
    ...
    register: async (d) => {
      ...
      const checkResult = await fetch(SUPABASE_URL + '/users?uid=eq.' + d.uid, { 
        method: 'GET',
        headers: supabaseHeaders
      });
      ...
      const result = await fetch(SUPABASE_URL + '/users', {
        method: 'POST',
        headers: supabaseHeaders,
        body: JSON.stringify({
          uid: d.uid,
    ...
    createPost: async (d) => {
      ...
      const result = await fetch(SUPABASE_URL + '/posts', {
        method: 'POST',
        headers: supabaseHeaders,
        body: JSON.stringify({
          id: postId,
    ...
    getPosts: async () => {
      const result = await fetch(SUPABASE_URL + '/posts?order=created_at.desc', { method: 'GET', headers: supabaseHeaders });
      if (!result.ok) return { success: false, data: [] };
      const posts = await result.json();
      const enrichedPosts = [];
      for (const post of posts) {
        const authorResult = await fetch(SUPABASE_URL + '/users?uid=eq.' + post.author_uid, { method: 'GET', headers: supabaseHeaders });
    ...
    searchUsers: async (d) => {
      ...
      const result = await fetch(SUPABASE_URL + '/users?uid=neq.' + user.uid, { method: 'GET', headers: supabaseHeaders });
      if (!result.ok) return { success: false, data: [] };
    ...
    sendFriendRequest: async (d) => {
      const user = await getCurrentUser();
      if (!user) return { success: false, message: '请先登录' };
      const result1 = await fetch(SUPABASE_URL + '/friendships?user_id=eq.' + user.uid + '&friend_id=eq.' + d.friendId, { method: 'GET', headers: supabaseHeaders });
      const result2 = await fetch(SUPABASE_URL + '/friendships?user_id=eq.' + d.friendId + '&friend_id=eq.' + user.uid, { method: 'GET', headers: supabaseHeaders });
      ...
      const result = await fetch(SUPABASE_URL + '/friendships', {
        method: 'POST',
        headers: supabaseHeaders,
        body: JSON.stringify({
          id: friendshipId,
    ...
    acceptFriendRequest: async (d) => {
      const result = await fetch(SUPABASE_URL + '/friendships?id=eq.' + d.friendshipId, {
        method: 'PATCH',
        headers: supabaseHeaders,
    ...
    deleteFriend: async (d) => {
      ...
      const result = await fetch(SUPABASE_URL + '/friendships?id=eq.' + friendshipId, { method: 'DELETE', headers: supabaseHeaders });
      const isSuccess = result.status >= 200 && result.status < 300;
      return { success: isSuccess, message: isSuccess ? '' : '操作失败' };
    },
    getFriends: async () => {
      const user = await getCurrentUser();
      if (!user) return { success: false, data: [] };
      const result1 = await fetch(SUPABASE_URL + '/friendships?user_id=eq.' + user.uid + '&is_accepted=eq.1', { method: 'GET', headers: supabaseHeaders });
      const result2 = await fetch(SUPABASE_URL + '/friendships?friend_id=eq.' + user.uid + '&is_accepted=eq.1', { method: 'GET', headers: supabaseHeaders });
      const json1 = await result1.json();
      const json2 = await result2.json();
      ...
      for (const friendship of allFriendships) {
        ...
        const friendResult = await fetch(SUPABASE_URL + '/users?uid=eq.' + friendUid, { method: 'GET', headers: supabaseHeaders });
    ...
    getFriendRequests: async () => {
      const user = await getCurrentUser();
      if (!user) return { success: false, data: [] };
      const result = await fetch(SUPABASE_URL + '/friendships?friend_id=eq.' + user.uid + '&is_accepted=eq.0', { method: 'GET', headers: supabaseHeaders });
      if (!result.ok) return { success: false, data: [] };
      const requests = await result.json();
      const enriched = [];
      for (const request of requests) {
        const requesterResult = await fetch(SUPABASE_URL + '/users?uid=eq.' + request.user_id, { method: 'GET', headers: supabaseHeaders });
    ...
    sendMessage: async (d) => {
      ...
      const result = await fetch(SUPABASE_URL + '/messages', {
        method: 'POST',
        headers: supabaseHeaders,
        body: JSON.stringify({
          id: messageId,
    ...
    getMessages: async (d) => {
      ...
      const result1 = await fetch(SUPABASE_URL + '/messages?sender_uid=eq.' + user.uid + '&receiver_uid=eq.' + friendId + '&order=created_at.asc', { method: 'GET', headers: supabaseHeaders });
      const result2 = await fetch(SUPABASE_URL + '/messages?sender_uid=eq.' + friendId + '&receiver_uid=eq.' + user.uid + '&order=created_at.asc', { method: 'GET', headers: supabaseHeaders });
      const json1 = await result1.json();
      const json2 = await result2.json();
      const messages = [];
    ...
    updateProfile: async (d) => {
      const user = await getCurrentUser();
      if (!user) return { success: false, message: '请先登录' };
      const result = await fetch(SUPABASE_URL + '/users?uid=eq.' + user.uid, {
        method: 'PATCH',
        headers: supabaseHeaders,
    ...
    changePassword: async (d) => {
      const user = await getCurrentUser();
      if (!user) return { success: false, message: '请先登录' };
      const checkResult = await fetch(SUPABASE_URL + '/users?uid=eq.' + user.uid, { method: 'GET', headers: supabaseHeaders });
      const checkJson = await checkResult.json();
      if (!checkResult.ok || checkJson.length === 0) {
    ...
    sendPasswordRequest: async (d) => {
      const checkResult = await fetch(SUPABASE_URL + '/users?uid=eq.' + d.uid, { method: 'GET', headers: supabaseHeaders });
      const checkJson = await checkResult.json();
      if (!checkResult.ok || checkJson.length === 0) {
        return { success: false, message: 'UID不存在' };
      }
      const requestCheck = await fetch(SUPABASE_URL + '/password_requests?uid=eq.' + d.uid + '&status=eq.pending', { method: 'GET', headers: supabaseHeaders });
      ...
      const result = await fetch(SUPABASE_URL + '/password_requests', {
        method: 'POST',
        headers: supabaseHeaders,
        body: JSON.stringify({
          id: requestId,
    ...
    getPasswordRequests: async () => {
      ...
      const result = await fetch(SUPABASE_URL + '/password_requests?order=created_at.desc', { method: 'GET', headers: supabaseHeaders });
      if (!result.ok) return { success: false, data: [] };
      const requests = await result.json();
      const enriched = [];
      for (const request of requests) {
        const userResult = await fetch(SUPABASE_URL + '/users?uid=eq.' + request.uid, { method: 'GET', headers: supabaseHeaders });
    ...
    handlePasswordRequest: async (d) => {
      ...
      const result = await fetch(SUPABASE_URL + '/password_requests?id=eq.' + d.requestId, {
        method: 'PATCH',
        headers: supabaseHeaders,
    ...
    getUsers: async () => {
      const user = await getCurrentUser();
      if (!user || !user.is_admin) return { success: false, data: [] };
      const result = await fetch(SUPABASE_URL + '/users?order=created_at.desc', { method: 'GET', headers: supabaseHeaders });
      if (!result.ok) return { success: false, data: [] };
